...
$destinatario = '<email@provedor.com>'; // Coloque o e-mail de destino da mensagem entre < e >
...
if(isset($emailEnviado) && $emailEnviado){ 
    echo "<script>alert(\"Sua mensagem foi enviada com sucesso! Aguarde retorno.\")</script>";
    header("refresh:1;url=index.html");
} else {
    // Senão, se deuErro existe e for TRUE...
    if(isset($deuErro) && $deuErro){ 
        echo "<script>alert(\"Houve um erro no envio, tente novamente mais tarde.\")</script>";
        header("refresh:1;url=index.html");
    } 
}
?>   
